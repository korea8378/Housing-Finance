    ...
    public List<ResDetailTotalAmountDto> selectGroupByYear() {
        String selectQuery =
                "SELECT new com.housing.finance.supportamount.dto.ResDetailTotalAmountDto(" +
                        "sa.year," +
                        "SUM(sa.amount)," +
                        "SUM(CASE WHEN sa.name = '주택도시기금' THEN sa.amount ELSE 0 END)," +
                        "SUM(CASE WHEN sa.name = '국민은행' THEN sa.amount ELSE 0 END)," +
                        "SUM(CASE WHEN sa.name = '우리은행' THEN sa.amount ELSE 0 END)," +
                        "SUM(CASE WHEN sa.name = '신한은행' THEN sa.amount ELSE 0 END)," +
                        "SUM(CASE WHEN sa.name = '한국시티은행' THEN sa.amount ELSE 0 END)," +
                        "SUM(CASE WHEN sa.name = '하나은행' THEN sa.amount ELSE 0 END)," +
                        "SUM(CASE WHEN sa.name = '농협은행/수협은행' THEN sa.amount ELSE 0 END)," +
                        "SUM(CASE WHEN sa.name = '외환은행' THEN sa.amount ELSE 0 END)," +
                        "SUM(CASE WHEN sa.name = '기타은행' THEN sa.amount ELSE 0 END)) " +
